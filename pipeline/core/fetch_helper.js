 ...
 * 4. noRetry option for expensive API calls (GPT Image, etc.)
 *
 * @param {string} url - The URL to fetch
 * @param {object} options - Standard fetch options
 * @param {object} retryConfig - Configuration for retry behavior
 * @param {boolean} retryConfig.noRetry - If true, disables all retries (for expensive API calls)
...
export async function fetchWithRetry(url, options = {}, retryConfig = {}) {
  ...
  while (attempt <= maxRetries) {
    ...
    if (fetchError) {
      // If noRetry mode, throw immediately on any error
      if (noRetry) throw fetchError;
      ...
      // If noRetry mode, return the response as-is (even on 429/5xx)
      if (noRetry) return response;
